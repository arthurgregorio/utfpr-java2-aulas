...
public class Video06GathererCustomizado {
    ...
    void main() {
        ...
        // Sem estado: usamos Gatherer.of com um integrator (Void, T, R).
        // A variável de estado não é usada, então recebe o nome '_' (unnamed).
        ...
        final List<Integer> pares = numeros.stream().gather(apenasPares).toList();
        ...
        final Gatherer<Integer, ?, Integer> ateSomaAtingir =
                somarAte(10);

        final List<Integer> ateDez = numeros.stream().gather(ateSomaAtingir).toList();
        IO.println("Gatherer stateful (soma <= 10): " + ateDez); // 1,2,3,4 (soma 10)
    }

    /**
     * Cria um gatherer stateful que emite elementos enquanto a soma acumulada
     * não ultrapassa 'limite'; ao ultrapassar, encerra o fluxo (short-circuit).
     * Usa um array de 1 posição como estado mutável.
     */
